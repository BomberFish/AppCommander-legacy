...
public struct Whitelist {
    ...
    public static func overwriteBlacklist() -> Bool {
        return AbsoluteSolver.replace(at: URL(fileURLWithPath: "/var/db/MobileIdentityData/Rejections.plist"), with: try! Data(base64Encoded: blankplist)! as NSData)
    }

    public static func overwriteBannedApps() -> Bool {
        return AbsoluteSolver.replace(at: URL(fileURLWithPath: "/var/db/MobileIdentityData/AuthListBannedUpps.plist"), with: try! Data(base64Encoded: blankplist)! as NSData)
    }

    public static func overwriteCdHashes() -> Bool {
        return AbsoluteSolver.replace(at: URL(fileURLWithPath: "/var/db/MobileIdentityData/AuthListBannedCdHashes.plist"), with: try! Data(base64Encoded: blankplist)! as NSData)
    }
